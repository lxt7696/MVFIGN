import argparse
import torch
from mul_deal_o import deal_dataset
import numpy as np
...
parser.add_argument('--round2', type=int, default=700)
...
def client_loss(alpha,beta,adj_batch,batch_output,x1):
    ...
    print(n,x1,(x1*x1)/n,pos_sum.mean())
...
def get_batch1(client,d):
    if client_adj[d][client].shape[0]>args.batch_size:
        ...
        features_batch = torch.stack(user_features[client])[rand_indx].cuda()
        adj_batch = client_adj[d][client][rand_indx,:][:,rand_indx]
        return features_batch, adj_batch
    else:
        return torch.stack(user_features[client]).cuda(),client_adj[d][client]
def get_batch2():
    sr=0.4
    print(len(at_f))
    rand_indx=torch.tensor(np.random.choice(np.arange(len(at_f)),int(len(at_f)*sr))).type(torch.long).cuda()
    return at_f[rand_indx].cuda(), torch.tensor(at_l)[rand_indx].cuda()
if __name__ == '__main__':
    ...
    for m in range(args.r):
        for t in range(args.round1):
            ...
            loss,x1[m],x2[m]=client_loss(alpha[m],beta[m],a_b.cuda(),batch_output,fx1)
    ...
    for epoch4 in tqdm(range(args.round2)):
        f_b1,a_b1=get_batch2()
        local_model1.train()
        optimizer1.zero_grad()
        batch_output= local_model1(f_b1)
        loss = F.nll_loss(batch_output, a_b1)
        loss.backward()
        optimizer1.step()
        tmp_test_acc= test(local_model1)
        if tt<tmp_test_acc:
            tt=tmp_test_acc
    log_file = open(r"arv1.txt", encoding="utf-8",mode="a+")  
