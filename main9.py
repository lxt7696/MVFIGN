import argparse
import torch
from deal9 import deal_dataset
import numpy as np
from model import Tr,GMLP
...
#get data(iid)
user_features,user_labels,client_adj,label_num,train_features,train_labels,test_features,test_labels,val_features,val_labels,at_f,at_l=deal_dataset(client_num,args.rate)
...
for i in range(client_num):
    ...
    local_model1.append(GMLP(nfeat=feature_dim,
            nhid=args.hidden,
            nclass=label_num,
...
global_model2=GMLP(nfeat=feature_dim,
            nhid=args.hidden,
            nclass=label_num,
            dropout=args.dropout)
global_model3=GMLP(nfeat=feature_dim,
            nhid=args.hidden,
            nclass=label_num,
            dropout=args.dropout)
optimizer3=optim.Adam(global_model3.parameters(),lr=args.lr, weight_decay=args.weight_decay)
if args.cuda:
    for i in range(client_num):
        local_model[i].cuda()
        local_model1[i].cuda()
...
def get_average(models,global_model):
    ...
    for key in weight_keys:#简单平均所有client的模型参数???横向联邦
        key_sum = 0
...
if __name__ == '__main__':
    ...
    for r in range(args.round1):
        ...
        for i in range(client_num):
            if args.cm==1:
                ou_t=get_out(i,global_model3)
            for t in range(args.epochs1):
                if args.cm==1 and r>0:
                    f_b,a_b=get_batch(i,ou_t,t)
                else:
                    f_b,a_b=get_batch1(i)
                local_model[i].train()
                optimizer[i].zero_grad()
                batch_output,fx1=local_model[i](f_b)
                loss,x1[i],x2[i]=client_loss(alpha[i],beta[i],a_b,batch_output,fx1)
                loss.backward(retain_graph=True)
                optimizer[i].step()   
        if args.d==1:  
                alpha,beta=get_alpha_beta(alpha,beta,x1,x2)
        print(r,alpha,beta)
        global_model1=get_average(local_model,global_model1)
        upload_local_model(global_model1,local_model)
        if args.cm==1:
            global_model3.tran1.load_state_dict(global_model1.state_dict())
            for name, param in global_model3.named_parameters():
                if "tran1" in name:
    ...
    for i in range(client_num):
        local_model1[i].tran1.load_state_dict(torch.load(PATH2))
    ...
    for epoch4 in tqdm(range(args.round2)):
        for i in range(client_num):
            for t in range(args.epochs2):
                ...
                batch_output= local_model1[i](torch.stack(train_features[i]).cuda())
                loss = F.nll_loss(batch_output, torch.tensor(train_labels[i]).cuda())
                loss.backward()
                optimizer1[i].step()
        global_model1=get_average(local_model1,global_model2)
        upload_local_model(global_model2,local_model1)
        val_acc=val(global_model2)
        tmp_test_acc= test(global_model2)
        if tt<tmp_test_acc:
            tt=tmp_test_acc
    log_file = open(r"log66.txt", encoding="utf-8",mode="a+")  
    with log_file as file_to_be_write:  
        print(args.c,args.d,args.cm,args.round1,tt, file=file_to_be_write)
