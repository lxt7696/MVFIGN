...
if __name__ == '__main__':
    ...
    # ====== 保存训练时间 ======
    log_time = open(r"mign_training_time.txt", encoding="utf-8", mode="a+")
    with log_time as f:
        print(f"数据集: {args.dataSet}", file=f)
        print(f"第一阶段: {phase1_time:.2f}秒", file=f)
        print(f"第二阶段: {phase2_time:.2f}秒", file=f)
        print(f"总时间: {total_train_time:.2f}秒", file=f)
        print("-" * 50, file=f)
    print("训练时间已保存到 mign_training_time.txt")
    # ====== 结束 ======
