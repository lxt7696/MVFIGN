...
def deal_dataset(client_num,rate,r):
    """
    【重要说明】这是 transductive 版本的数据加载函数
    
    参数 r 的含义：
    - r=1: 只使用 2-hop 邻接矩阵（对应 Table 5 的 "Our Method (r=2)"）
    - r=4: 使用 2,3,4,5-hop 邻接矩阵（对应 Table 5 的 "Our Method (r=1,2,3,4)"）
    
    使用方法：
    - 复现 "Our Method (r=2)": python mul_main.py --r 1
    - 复现 "Our Method (r=1,2,3,4)": python mul_main.py --r 4
    """
...
#######################################
    ...
    # 【关键修改2】固定只生成一个 r=2 的邻接矩阵（不是循环）
    adj_list=[]
    for i in range(client_num):
        adj1=adj.todense()  # 使用完整图
        adj_sp=sp.csr_matrix(adj1)
        adj_normalizer = fetch_normalization("AugNormAdj")
        adj2 = adj_normalizer(adj_sp) 
        adj3 = sparse_mx_to_torch_sparse_tensor(adj2)
        
        # 固定使用 2-hop（r=2 对应 get_A_r 的参数是 2）
        adj4=get_A_r(adj3, 2)
        adj_list.append((adj4))
    
    # 【关键修改3】返回的 adj_list_list 只包含一个列表（单一的 r=2）
    # 这与原始的 mul_deal_tran.py 不同，原始版本会循环生成多个 r
    # 【关键修改3】为了兼容 mul_main.py 中的循环，仍需要保持列表结构
    # 但实际上每个client只有一个相同的邻接矩阵（r=2）
    # 这里复制多次以兼容原有的 for m in range(args.r) 循环
    adj_list_list = [adj_list]  # 注意这里只有一层列表
    for _ in range(r):  # 虽然都是相同的 r=2 邻接矩阵，但复制 r 次
        adj_list_list.append(adj_list)
