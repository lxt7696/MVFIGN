...
if __name__ == '__main__':
    ...
    print(f"第一阶段：训练{client_num}个本地模型...")
    # ====== 结束 ======
    ...
    print(f"第一阶段训练完成，用时: {phase1_time:.2f}秒")
    # ====== 结束 ======
    ...
    print(f"\n第二阶段训练完成，用时: {phase2_time:.2f}秒")
    print(f"总训练时间: {total_train_time:.2f}秒")
    print(f"  - 第一阶段: {phase1_time:.2f}秒")
    print(f"  - 第二阶段: {phase2_time:.2f}秒")
    # ====== 结束 ======
    ...
    for name, param in local_model1[0].named_parameters():
            print(name,param)
    # ====== 保存训练时间 ======
    log_time = open(r"mvfign_training_time.txt", encoding="utf-8", mode="a+")
    with log_time as f:
        print(f"数据集: {args.dataSet}, 客户端数: {client_num}", file=f)
        print(f"第一阶段(本地训练): {phase1_time:.2f}秒", file=f)
        print(f"第二阶段(全局训练): {phase2_time:.2f}秒", file=f)
        print(f"总训练时间: {total_train_time:.2f}秒", file=f)
        print("-" * 50, file=f)
    print("训练时间已保存到 mvfign_training_time.txt")
    # ====== 结束 ======
