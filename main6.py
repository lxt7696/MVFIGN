import argparse
import torch
from deal8 import deal_dataset
import numpy as np
...
parser.add_argument('--epochs2', type=int, default=1)
...
#get data(iid)
user_features,client_adj,label_num,train_features,train_labels,test_features,test_labels,val_features,val_labels,at_f,at_l=deal_dataset(args.alpha,client_num,args.rate)
...
def val(model):
    ...
    print(acc_test,micro)
    return acc_test
def get_batch(client,class_num):
    if len(user_batch_index[client][class_num])>args.batch_size:
        rand_indx = torch.tensor(np.random.choice(user_batch_index[client][class_num], args.batch_size)).type(torch.long).cuda()
        ...
        return features_batch, adj_batch
          ...
          return torch.stack(user_features[client])[user_batch_index[client][class_num]],client_adj[client][user_batch_index[client][class_num],:][:,user_batch_index[client][class_num]]
...
if __name__ == '__main__':
    ...
    for epoch4 in tqdm(range(args.round2)):
        ...
        if tt<tmp_test_acc:
            tt=tmp_test_acc
    log_file = open(r"log56.txt", encoding="utf-8",mode="a+")  
