...
from mpc_utils_time_del import MPSPDZManager
from he_utils_time_improve import GradientAggregator
...
class SecureGMLP(nn.Module):
    ...
    def aggregate_gradients_he(self, local_gradients):
        ...
        Returns:
            ...
            he_time: dict with keys {'total', 'encrypt', 'aggregate', 'decrypt'} 或 float
        """
        if not self.use_he or self.grad_aggregator is None:
            print(f"[SecureGMLP] HE not enabled, skipping HE aggregation")
            return None, {'total': 0.0, 'encrypt': 0.0, 'aggregate': 0.0, 'decrypt': 0.0}
        
        # grad_aggregator.aggregate_gradients 返回 (grads, time_info)
        # time_info 可能是字典或float
        result = self.grad_aggregator.aggregate_gradients(local_gradients)
        
        if isinstance(result, tuple) and len(result) == 2:
            grads, time_info = result
            # 如果已经是字典，直接返回
            if isinstance(time_info, dict):
                return grads, time_info
            # 如果是float，转换为字典
            elif isinstance(time_info, (int, float)):
                return grads, {'total': float(time_info), 'encrypt': 0.0, 'aggregate': 0.0, 'decrypt': 0.0}
            else:
                return grads, {'total': 0.0, 'encrypt': 0.0, 'aggregate': 0.0, 'decrypt': 0.0}
        else:
            return result, {'total': 0.0, 'encrypt': 0.0, 'aggregate': 0.0, 'decrypt': 0.0}