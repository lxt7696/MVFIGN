import argparse
import torch
from deal7 import deal_dataset
import numpy as np
...
#get data(iid)
user_features,client_adj,label_num,train_features,train_labels,test_features,test_labels,val_features,val_labels,at_f,at_l=deal_dataset(client_num,args.rate)
...
def client_loss(alpha,beta,adj_batch,batch_output,x1):
    ...
    print((x1*x1)/n,pos_sum.mean(),x1*x1,pos_sum.sum())
    return loss,(x1*x1)/n,pos_sum.mean()
    #return loss,(x1*x1),pos_sum.sum()
...
if __name__ == '__main__':
    ...
    for r in range(args.round1):
        ...
        for i in range(client_num):
            for t in range(args.epochs1):
                ...
                optimizer[i].step()   
        if args.d==1:  
                beta=get_beta(alpha,beta,x1,x2)
        print(r,alpha,beta)
    ...
    for epoch4 in tqdm(range(args.round2)):
        ...
        if tt<tmp_test_acc:
            tt=tmp_test_acc
    log_file = open(r"log25.txt", encoding="utf-8",mode="a+")  
