...
def deal_dataset(client_num,rate,r):
    ...
    tdata_dict = tdataset[0]  # 获取字典数据     # 使用SimpleNamespace进行轻量级转换
    tdata = types.SimpleNamespace(**tdata_dict)
    ...
    c_e_num=[i*edge_num for i in [0,0.5,1]]
    e_t=np.random.permutation(edge_num)
    c_edge_index=[]
    for i in range(client_num):
        c_edge_index.append(e_t[int(c_e_num[i]):int(c_e_num[i+1])])
        ee=np.array(adj.todense()).nonzero()[0][c_edge_index[i]].tolist()
        ee1=np.array(adj.todense()).nonzero()[1][c_edge_index[i]].tolist()
        user_data_index.append(list(set(ee+ee1))[:])
...
#self
    ...
    for i in range(client_num):
        user_data_list=user_data_index[i]
    ...
    #class_index=[]
    #class_index.append(torch.nonzero(tdata.y==0).squeeze())
    #class_index.append(torch.nonzero(tdata.y==1).squeeze())
    #class_index.append(torch.nonzero(tdata.y==2).squeeze())
    #user_batch_index=[]
    #for i in range(client_num):
      #  tu=[]
        #for j in range(label_num):
          #  nn=int(len(user_data_index[i])/label_num)
            #tu.append([j1 for j1 in range(nn*j,nn*(j+1))])
        #user_batch_index.append(tu[:])
    return user_features,user_labels,adj_list,label_num,train_features,train_labels,test_features,test_labels,val_features,val_labels,at_features,at_labels
