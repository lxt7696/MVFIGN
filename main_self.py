...
parser.add_argument('--round1', type=int, default=200)
...
parser.add_argument('--batch_size1', type=int, default=30,
                    help='batch size')
parser.add_argument('--outputdim', type=int, default=256)
...
for i in range(client_num):
    local_model.append(Mlp(feature_dim,args.outputdim,args.dropout))
    local_model1.append(GMLP(args.outputdim,args.outputdim,label_num,args.dropout))
...
global_model1=Mlp(feature_dim,args.outputdim,args.dropout)
global_model2=GMLP(args.outputdim,args.outputdim,label_num,args.dropout)
if args.cuda:
    for i in range(client_num):
        local_model[i].cuda()
        local_model1[i].cuda()
    global_model2.cuda()
def client_loss(alpha,beta,batch_adj,batch_output,batch_input):
    aij_matrix=torch.sub(batch_output,batch_input).float()
    ...
    for key in batch_adj:
        for value in batch_adj[key]:
            ...
            x2_list.append(t*t)
    x2=sum(x2_list)
    loss=alpha*x1*x1+beta*x2*0.5
    return loss,x1*x1,x2
    ...
    for r in range(args.round1):
        ...
        for i in range(client_num):
            for t in range(args.epochs1):
                ...
                loss,x1[i],x2[i]=client_loss(alpha[i],beta[i],adj_batch,batch_output,feature_batch)
                loss.backward()
                optimizer[i].step()     
