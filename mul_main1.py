import argparse
import torch
from mul_deal1 import deal_dataset
import numpy as np
...
parser.add_argument('--d', type=int, default=2,
                    help='learning rate.')
parser.add_argument('--cm', type=float, default=0.1,
                    help='learning rate.')
...
def get_batch1(client):
    print(client_adj[client].shape[0])
...
if __name__ == '__main__':
    ...
    for epoch4 in tqdm(range(args.round2)):
        ...
        if tt<tmp_test_acc:
            tt=tmp_test_acc
    log_file = open(r"log106.txt", encoding="utf-8",mode="a+")  
