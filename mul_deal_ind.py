...
def get_A_r(adj, r):
    ...
    if r == 1:
        ...
        adj_label = adj_label@adj_label@adj_label@adj_label@adj_label@adj_label@adj_label@adj_label@adj_label@adj_label@adj_label@adj_label@adj_label@adj_label@adj_label@adj_label@adj_label@adj_label@adj_label@adj_label
    return adj_label
...
#######################################
    ...
    train_index=n_r[:int(0.6*data_num)].tolist()
    test_index=n_r[int(0.6*data_num):int(0.8*data_num)].tolist()
    val_index=n_r[int(0.6*data_num):].tolist()
    ...
    for j in range(r):
        ...
        for i in range(client_num):
            adj1=adj.todense()[train_index,:][:,train_index]
            adj_val=adj.todense()[val_index,:][:,val_index]
            adj_sp=sp.csr_matrix(adj1)
            adj_sp_val=sp.csr_matrix(adj_val)
            adj_normalizer = fetch_normalization("AugNormAdj")
            adj2 = adj_normalizer(adj_sp) 
            adj2_val = adj_normalizer(adj_sp_val) 
            adj3 = sparse_mx_to_torch_sparse_tensor(adj2)
            adj3_val = sparse_mx_to_torch_sparse_tensor(adj2_val)
            adj4=get_A_r(adj3,j+2)
            adj4_val=get_A_r(adj3_val,j+2)
            adj_val_list.append((adj4_val).cuda())
...
#self
    ...
    return user_features,user_labels,adj_list_list,adj_val_list,label_num,train_features,train_labels,test_features,test_labels,val_features,val_labels,at_features,at_labels
