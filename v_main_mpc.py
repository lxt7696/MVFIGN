# v_main_mpc.py - 完整版（集成分块 MPC）

import argparse
import torch
from v_deal import deal_dataset
import numpy as np
from v_model_mpc import SecureTr, SecureGMLP, Tr, GMLP
...
parser = argparse.ArgumentParser(description='pytorch version of TEE-VFL with MPC')
...
parser.add_argument('--mpc_port', type=int, default=6000,
...
def client_loss(alpha, beta, adj_batch, batch_output, x1):
    """计算客户端损失"""
    print(f"\n[client_loss] ========== START ==========")
    print(f"[client_loss] batch_output shape: {batch_output.shape}")
    print(f"[client_loss] x1 value: {x1}")
    print(f"[client_loss] alpha: {alpha}, beta: {beta}")
    ...
    print(f"[client_loss] term1 (x1*x1)/n: {((x1*x1)/n).item():.4f}")
    print(f"[client_loss] term2 pos_sum.mean(): {pos_sum.mean().item():.4f}")
    print(f"[client_loss] total loss: {loss.item():.4f}")
    print(f"[client_loss] ========== END ==========\n")
...
if __name__ == '__main__':
    ...
    for m in range(client_num):
        ...
        for r in range(args.round1):  # 移除 tqdm 以便看清楚输出
            print(f"\n[Client {m}] ========== Round {r+1}/{args.round1} START ==========")
            
            x1 = [0.0 for x in range(client_num)]
            x2 = [0.0 for x in range(client_num)]
            
            print(f"[Client {m}] Getting batch...")
            f_b, a_b = get_batch1(m)
            print(f"[Client {m}] Got batch: features shape={f_b.shape}, adj shape={a_b.shape}")
            
            local_model[m].train()
            optimizer[m].zero_grad()
            
            # 前向传播
            print(f"[Client {m}] Forward pass...")
            if args.use_mpc and isinstance(local_model[m], SecureTr):
                print(f"[Client {m}] Using SecureTr with MPC")
                batch_output, fx1 = local_model[m](f_b, compute_norm=True)
            else:
                print(f"[Client {m}] Using regular Tr")
                batch_output, fx1 = local_model[m](f_b)
            
            print(f"[Client {m}] Forward done: output shape={batch_output.shape}, fx1={fx1.item() if hasattr(fx1, 'item') else fx1}")
            
            # 计算损失
            print(f"[Client {m}] Computing loss...")
            loss, x1[m], x2[m] = client_loss(alpha[m], beta[m], a_b, 
                                            batch_output, fx1)
            print(f"[Client {m}] Loss={loss.item():.6f}, x1[{m}]={x1[m]}, x2[{m}]={x2[m]}")
            
            # 反向传播
            print(f"[Client {m}] Backward pass...")
            loss.backward(retain_graph=True)
            optimizer[m].step()
            print(f"[Client {m}] ========== Round {{r+1}}/{{args.round1}} DONE ==========\n")
    ...
    # 保存结果
    log_file = open(r"log_mpc.txt", encoding="utf-8", mode="a+")
    with log_file as file_to_be_write:
        print(f"Party{args.party_id} MPC={args.use_mpc} c={args.c} "
              f"cm={args.cm} round1={args.round1} batch_size={args.mpc_batch_size} "
              f"acc={tt:.4f}", 
              file=file_to_be_write)
    
    # 详细日志
    log_file2 = open(r"results_mpc.txt", encoding="utf-8", mode="a+")
    with log_file2 as f:
        print(f"\n{'='*60}", file=f)
